...
// File: contracts/Ontact Protocol.sol
...
contract CROS is
    ...
    Freezable
{
    string constant private _name = "Grinbit";
    string constant private _symbol = "GRBT";
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "GRBT/transfer : Should not send to zero address"
        );
        ...
        returns (bool success)
    {
        require(
            to != address(0),
            "CROS/transferFrom : Should not send to zero address"
        );
        ...
        _approve(
            ...
            _allowances[from][msg.sender].sub(
                amount,
                "CROS/transferFrom : Cannot send more than allowance"
            )
        );
        success = true;
    }
        ...
        returns (bool success)
    {
        require(
            spender != address(0),
            "CROS/approve : Should not approve zero address"
        );
